...
report_results<-function(kmer_hist,kmer_hist_orig, k, container, foldername)
{
	...
	if(!is.null(container[[1]])) 
    {
       ...
       het_hist <- (2 * (1 - amd) * (1 - (1 - ahet)^k))                         * dnbinom(x, size = akcov     / adups, mu = akcov)     * amlen 
       hom_hist <- ((amd * (1 - (1 - ahet)^k)^2) + (1 - amd) * ((1 - ahet)^k))  * dnbinom(x, size = akcov * 2 / adups, mu = akcov * 2) * amlen

       hom_kmers = sum(as.numeric(x*hom_hist))
       het_kmers = sum(as.numeric(x*het_hist))
       repeat_kmers = total_kmers - hom_kmers - het_kmers - total_error_kmers
       
       repeat_len=repeat_kmers/(2*kcov)
       unique_len=(hom_kmers+het_kmers)/(2*kcov)
       ...
       lines(x, hom_hist, col=COLOR_2PEAK, lty=1, lwd=3)
       lines(x, het_hist, col=COLOR_2PEAK, lty=1, lwd=3)
       ...
       my_list <- list("hom" = hom_hist, "het" = het_hist, "error" = error_kmers)
       return(my_list)
...
if(1==2) {
    ...
    ## values for testing
    histfile <- "./corrected_pacbio.histo"
    k <- 21
    readlength <- 10000
    foldername <- "test_hack1"
	...
	result_df <- report_results(kmer_prof,kmer_prof_orig, k, best_container, foldername)
}
